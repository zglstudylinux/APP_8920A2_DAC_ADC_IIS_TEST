...
config(LIST, "测试模式", "IIS_测试模式", test_mode, 4, "PCM2DAC", "AUX_ADC2DAC", "AUX_ADC2IISSRCTX", "IISRX2DAC", 0);
config(LEVEL, 0x00);
...
config(BIT, "预置MIX_CAP", "设置预置RF参数的MIX_CAP，用于调平TX功率", BT_RF_MIX_CAP, 4, 7, 9, 9);
...
config(NUM, "SNDP单麦降噪量", "声加算法降噪量（0~58dB,步进2dB）", BT_SNDP_LEVEL, 0, 29, 10, BT_SCO_EN);
config(CHECK, "SNDP双麦功能", "是否支持双唛功能", BT_SNDP_DM_EN, 0, BT_SCO_EN);
config(BYTE, "双麦间距", "出音孔之间的直线距离（20~40mm）", BT_SNDP_DM_DISTANCE, 20, 40, 23, BT_SNDP_DM_EN);
config(BYTE, "拾音角度", "通话的拾音角度（与双麦之间的连线左右两边形成的夹角（10-89度 步进5度）），如果耳机佩戴位置变化大，建议加大角度", BT_SNDP_DM_DEGREE, 0, 16, 4, BT_SNDP_DM_EN);
config(BYTE, "最大保护角", "（10-89度 步进5度），默认89度；在调节拾音角度bf效果无法改善情况下可以调小此角度，但一定大于等于拾音角度", BT_SNDP_DM_MAX_DEGREE, 0, 16, 16, BT_SNDP_DM_EN);
config(BYTE, "通话降噪量", "通话角度的语音降噪量（0~58dB 步进2dB）", BT_SNDP_DM_LEVEL, 0, 29, 10, BT_SNDP_DM_EN);
config(BYTE, "风噪降噪量", "抑制风噪的降噪量，一般大于通话降噪量（0~58dB 步进2dB）", BT_SNDP_DM_WIND_LEVEL, 0, 29, 10, BT_SNDP_DM_EN);
